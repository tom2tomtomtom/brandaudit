...
@brand_audit_bp.route("/report/generate-professional", methods=["POST"])
@jwt_required()
def generate_professional_report():
    """Generate professional report with multiple templates and formats"""
    try:
        # Get current user
        current_user_id = get_jwt_identity()
        user = User.query.get(current_user_id)
        ...
        template = data.get('template', 'detailed_analysis')
        theme = data.get('theme', 'corporate_blue')
        export_formats = data.get('export_formats', ['pdf'])

        # Validate template and theme
        valid_templates = ['executive_summary', 'detailed_analysis', 'presentation_deck', 'consulting_report', 'strategic_brief']
        valid_themes = ['corporate_blue', 'modern_minimal', 'consulting_premium', 'brand_focused', 'executive_dark']
        valid_formats = ['pdf', 'pptx', 'html']

        if template not in valid_templates:
            return jsonify({"success": False, "error": f"Invalid template. Must be one of: {valid_templates}"}), 400

        if theme not in valid_themes:
            return jsonify({"success": False, "error": f"Invalid theme. Must be one of: {valid_themes}"}), 400

        if not export_formats or not all(fmt in valid_formats for fmt in export_formats):
            return jsonify({"success": False, "error": f"Invalid export formats. Must be from: {valid_formats}"}), 400

        # Generate professional report
        try:
            from src.services.presentation_service import ProfessionalPresentationService, ReportTemplate, ReportTheme
            ...
            if report_result.get('errors'):
                current_app.logger.warning(f"Professional report generation had errors for {user.email}: {report_result['errors']}")
            ...
            response_data = {
                ...
                "theme": theme,
                "export_formats": export_formats,
                "generation_timestamp": report_result.get('generation_timestamp'),
                "reports_generated": report_result.get('reports_generated', {}),
                "charts_generated": report_result.get('charts_generated', []),
                "slide_count": report_result.get('slide_count', 0),
                "capabilities_used": report_result.get('capabilities_used', {}),
                "errors": report_result.get('errors', [])
            }

            current_app.logger.info(f"Professional report generated by user {user.email} for brand: {brand_name}")
            current_app.logger.info(f"Template: {template}, Theme: {theme}, Formats: {export_formats}")
            ...
            current_app.logger.error(f"Professional report generation error for {user.email}: {str(e)}")
            return jsonify(
                {
                    "success": False,
        ...
        current_app.logger.error(f"Professional report generation error: {str(e)}")
        return jsonify({"success": False, "error": "Failed to generate professional report"}), 500
