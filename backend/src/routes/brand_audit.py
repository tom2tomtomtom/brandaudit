...
from src.models.user import User, Analysis, UploadedFile, db
from src.schemas.validation import (
    BrandSearchSchema, BrandAssetsSchema, AnalysisRequestSchema,
    FileUploadSchema, ReportGenerationSchema, validate_file_upload,
    sanitize_filename, validate_analysis_id
)
...
def search_brand():
    ...
    try:
        ...
        current_app.logger.info(f'Brand search request from user {user.email}: {data["query"]}')
...
def get_brand_assets():
    ...
    try:
        ...
        current_app.logger.info(f'Brand assets request from user {user.email}: {data["website"]}')
...
def upload_files():
    ...
    try:
        ...
        current_app.logger.info(f'Files uploaded by user {user.email}: {len(uploaded_files)} files')
...
def analyze_brand():
    ...
    try:
        ...
        thread = threading.Thread(
            target=run_analysis,
            args=(analysis_id, data, current_user_id)
        )
        thread.daemon = True
        thread.start()
...
def get_user_analyses():
    ...
    try:
        ...
        # Get user's analyses
        analyses = Analysis.query.filter_by(user_id=current_user_id)\
    ...
    try:
        with current_app.app_context():
            ...
            if analysis_data.get('website'):
                brand_info = brand_data_service.get_company_info(analysis_data['company_name'])
            ...
            if analysis_data['analysis_options'].get('pressCoverage'):
                ...
                news_analysis = news_service.get_brand_news(analysis_data['company_name'])
            ...
            if results.get('news_analysis', {}).get('sentiment_score', 0) > 0.5:
                score += 10
            if results.get('llm_insights', {}).get('sentiment', 'neutral') == 'positive':
                score += 15
