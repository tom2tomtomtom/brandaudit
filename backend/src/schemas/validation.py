...
class BrandSearchSchema(Schema):
    ...
    query = fields.Str(
        ...
        validate=[
            ...
            validate.Regexp(r'^[a-zA-Z0-9\s\-&.,()]+$', error='Invalid characters in query')
        ]
    )
...
class AnalysisRequestSchema(Schema):
    ...
    company_name = fields.Str(
        ...
        validate=[
            validate.Length(min=1, max=200),
            validate.Regexp(r'^[a-zA-Z0-9\s\-&.,()]+$', error='Invalid characters in company name')
        ]
    )
...
class ReportGenerationSchema(Schema):
    ...
    format = fields.Str(
        required=True,
        validate=validate.OneOf(['pdf', 'html', 'json'])
    )
